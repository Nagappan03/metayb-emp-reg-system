...
function EmployeeLogin() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        const newAuth = { isAuthenticated: true, role: 'employee', employeeId: result.employeeId };
        setAuth(newAuth);
        sessionStorage.setItem('auth', JSON.stringify(newAuth));
        navigate('/artCreation');
      } else {
  ...
  return (
    <Box display="flex" justifyContent="center" alignItems="center" minHeight="100vh">
      ...
      >
        <Typography component="h1" variant="h5" sx={{ mb: 2, textAlign: 'center' }}>Employee Login</Typography>
        <TextField 
          name="email" 
          type="email" 
          label="Email Address" 
